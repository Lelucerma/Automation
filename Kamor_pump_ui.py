...
class Ui_Form(object):
    ...
    def setupUi(self, Form):
        ...
        # 开始溶胀
        self.Thread.swell_start.this.connect(self.swell_start)
        # 开始第一个单元控制
        self.Thread.unit1_start.clicked.connect(self.unit1_start)
        # 开始第二个单元控制
        self.unit2_start.clicked.connect(self.unit2_start)
        # 开始第二个单元控制
        self.wash_start.clicked.connect(self.wash_start)
        # 压力传感器的控制
        self.pressure_start.clicked.connect(self.pressure_start)
        # 压力数值的显示
        self.pressure_display.clicked.connect(self.pressure_display)
        # 文本框清除按钮
    ...
    def pump_open_button(self, add):
        self.slave_add = add
        if self.slave_add == 3:
            self.speed = int(int(self.pump3_spinbox.text()) * 140)
            self.first = '3 pump open'
        elif self.slave_add == 4:
            self.speed = int(int(self.pump4_spinbox.text()) * 140)
            self.first = '4 pump open'
        elif self.slave_add == 5:
            self.speed = int(int(self.pump5_spinbox.text()) * 140)
            self.first = '5 pump open'
        elif self.slave_add == 6:
            self.speed = int(int(self.pump6_spinbox.text()) * 140)
            self.first = '6 pump open'
            self.newline(self.speed)
        elif self.slave_add == 7:
            self.speed = int(int(self.pump7_spinbox.text()) * 140)
            self.first = '7 pump open'
            self.newline(self.speed)
        elif self.slave_add == 8:
            self.speed = int(int(self.pump8_spinbox.text()) * 140)
            self.first = '8 pump open'
            self.newline(self.speed)
        elif self.slave_add == 9:
            self.speed = int(int(self.pump9_spinbox.text()) * 140)
            self.first = '9 pump open'
            self.newline(self.speed)
        elif self.slave_add == 10:
            self.speed = int(int(self.pump10_spinbox.text()) * 140)
            self.first = '10 pump open'
            self.newline(self.speed)
        elif self.slave_add == 11:
            self.speed = int(int(self.pump11_spinbox.text()) * 140)
